...
type ApiGenerateSsoTokenRequest struct {
	ctx _context.Context
	ApiService *UsersApiService
	xRequestId *string
	xTraceId *string
...
// [Uuid4](https://en.wikipedia.org/wiki/Universally_unique_identifier#Version_4_(random)) to identify individual requests for support cases. You can use [uuidgenerator](https://www.uuidgenerator.net/version4) to generate them manually.
func (r ApiGenerateSsoTokenRequest) XRequestId(xRequestId string) ApiGenerateSsoTokenRequest {
	r.xRequestId = &xRequestId
	return r
}
// Identifier to trace group of requests.
func (r ApiGenerateSsoTokenRequest) XTraceId(xTraceId string) ApiGenerateSsoTokenRequest {
	r.xTraceId = &xTraceId
	return r
}

func (r ApiGenerateSsoTokenRequest) Execute() (FindSsoTokenResponse, *_nethttp.Response, error) {
	return r.ApiService.GenerateSsoTokenExecute(r)
}

/*
GenerateSsoToken Generate one time SSO token

Generate an one time SSO token for the authenticated user account.

 @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiGenerateSsoTokenRequest
*/
func (a *UsersApiService) GenerateSsoToken(ctx _context.Context) ApiGenerateSsoTokenRequest {
	return ApiGenerateSsoTokenRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *UsersApiService) GenerateSsoTokenExecute(r ApiGenerateSsoTokenRequest) (FindSsoTokenResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodPost
		localVarPostBody     interface{}
		localVarFormFileName string
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  FindSsoTokenResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "UsersApiService.GenerateSsoToken")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/users/sso-token"
...
// Filter if user is default account or not.
func (r ApiRetrieveUserListRequest) Default_(default_ bool) ApiRetrieveUserListRequest {
	r.default_ = &default_
	return r
}
...
// Full text search when listing the users. Can be searched by &#x60;first name&#x60;, &#x60;last name&#x60;, &#x60;email&#x60; or &#x60;first name&#x60; and &#x60;last name&#x60; combined
func (r ApiRetrieveUserListRequest) Search(search string) ApiRetrieveUserListRequest {
	r.search = &search
	return r
}
